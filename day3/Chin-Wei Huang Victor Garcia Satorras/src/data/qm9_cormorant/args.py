...
def setup_shared_args(parser):
    ...
    parser.add_argument(
        "--shuffle", action=BoolArg, default=True, help="Shuffle minibatches."
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--log-level", type=str, default="info", help="Logging level to output"
    )
    ...
    parser.add_argument(
        "--no-cuda", "--cpu", dest="cuda", action="store_false", help="Use CPU"
    )
...
def setup_argparse(dataset):
    ...
    parser = argparse.ArgumentParser(
        description="Cormorant network options for the md17 dataset."
    )
